...
void convertir(int in_fd, int out_fd, BMPHeader h, BMPInfoHeader infoh, int start, int end, int pos) 
{
    ...
    printf("WIDTH = %d\n", width);
    printf("HEIGTH = %d\n", height);
    printf("pos*width*height = %d\n",pos*width*height);

    lseek(in_fd,sizeof(BMPHeader)+ sizeof(BMPInfoHeader) + pos*width*height,SEEK_SET);
    ...
    lseek(out_fd, sizeof(BMPHeader)+ sizeof(BMPInfoHeader) + pos*width*height, SEEK_SET);
...
int main() 
{
    ...
    int shm_fd, shm_fdA, shm_fdB, shm_fdC, fileOpened;
    void *ptr;
    void *ptrA;
    void *ptrB;
    void *ptrC;
    ...
    if(pidA == 0){
        ...
        fflush(0);
        convertir(in_fd, out_fd, h, infoh, 0, 354, 0);
    } else if(pidA != 0){
        pidB = fork();
        if(pidB == 0){
            // //codigo hijo b
            ...
            // int in_fdA,out_fdA;
            // fflush(0);
            // in_fdA = open(BMP_FILE, O_RDONLY);
            // out_fdA = open(GRAYSCALE_FILE, O_WRONLY, 0644);
            // convertir(in_fdA, out_fdA, h, infoh, 0, 354, 1);
        } else if(pidB != 0){
            pidC = fork();
            if(pidC == 0){
                //codigo hijo c
                ...
                // fflush(0);
                // convertir(in_fdC, out_fdC, h, infoh, 0, 354, 2);
            } else if(pidC != 0){                    
                    sleep(3);
                    wait(NULL);
                    wait(NULL);
