...
void convertir(int in_fd, int out_fd, BMPHeader h, BMPInfoHeader infoh, int cantPixeles, int pos) 
{
	...
	int espacio_ocupado = cantPixeles*cantPixeles*9;

    lseek(in_fd,sizeof(BMPHeader)+ sizeof(BMPInfoHeader) + pos*espacio_ocupado,SEEK_SET);
    ...
    lseek(out_fd, sizeof(BMPHeader)+ sizeof(BMPInfoHeader) + pos*espacio_ocupado, SEEK_SET);
    write(out_fd, &nueva_imagen[0], espacio_ocupado);
}
...
int main() 
{
    ...
    if(pidA == 0){
        ...
        convertir(in_fd, out_fd, h, infoh, 354, 0);
    } else if(pidA != 0){
        pidB = fork();
        if(pidB == 0){
            //codigo hijo b
            ...
            convertir(in_fdB, out_fdB, h, infoh, 354, 1);
        } else if(pidB != 0){
            pidC = fork();
            if(pidC == 0){
                //codigo hijo c
                ...
                convertir(in_fdC, out_fdC, h, infoh, 354, 2);
            } else if(pidC != 0){                    
                    wait(NULL);
                    wait(NULL);
                    wait(NULL);
